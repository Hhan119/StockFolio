    ...
    <section className="grid gap-4">
      <header className="rounded-2xl border border-slate-800 bg-slate-950 p-5 text-white shadow-sm">
        ...
        <div className="mt-2 flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          ...
          <Link className="rounded-2xl bg-slate-800 px-4 py-3 text-sm font-black text-white hover:bg-slate-700" to="/portfolio/holdings">
            보유종목 화면으로 이동
          </Link>
        </div>
      </header>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_380px]">
        <main className="grid gap-4">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
              <div className="grid content-start gap-3">
                <h3 className="text-xl font-black text-slate-950">1. 포트폴리오 정보</h3>
              ...
              <div className="grid content-start gap-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <h3 className="text-xl font-black text-slate-950">2. 종목 검색 후 담기</h3>
                  <div className="grid grid-cols-3 rounded-2xl bg-slate-100 p-1 text-xs font-black">
                    {[["ALL", "전체"], ["KR", "국내"], ["US", "해외"]].map(([value, label]) => (
                      <button
                        className={`rounded-xl px-3 py-2 ${searchMarket === value ? "bg-slate-950 text-white" : "text-slate-500"}`}
                        key={value}
                        onClick={() => {
                ...
                <form className="grid gap-2 sm:grid-cols-[1fr_auto]" onSubmit={searchStocks}>
                  ...
                  />
                  <button className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-black text-slate-950 hover:bg-cyan-300" disabled={searching}>
                    {searching ? "검색 중" : "검색"}
                  </button>
                </form>

                <div className="grid max-h-52 gap-2 overflow-y-auto rounded-2xl border border-slate-200 bg-slate-50 p-2">
                  {results.map((item) => (
                    <button
                      className={`rounded-xl p-3 text-left transition ${selectedStock?.ticker === item.ticker ? "bg-slate-950 text-white" : "bg-white text-slate-900 hover:bg-cyan-50"}`}
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <span className={selectedStock?.ticker === item.ticker ? "text-xs font-bold text-slate-400" : "text-xs font-bold text-slate-500"}>
                  ...
                  {!results.length && <p className="p-3 text-sm font-bold text-slate-500">검색하면 여기에 종목 목록이 표시됩니다.</p>}
                </div>

                {selectedStock && (
                  <form className="rounded-2xl border border-slate-200 bg-white p-4" onSubmit={addDraftHolding}>
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <strong className="block text-lg font-black text-slate-950">{selectedStock.name}</strong>
                        <span className="text-sm font-bold text-slate-500">{selectedStock.ticker} · 현재가 {formatMoney(selectedStock.currentPrice, selectedStock.currency)}</span>
                      </div>
                      <button className="text-sm font-black text-slate-400 hover:text-slate-950" onClick={() => setSelectedStock(null)} type="button">
                        취소
                      </button>
                    </div>
                    <div className="mt-3 grid gap-2 sm:grid-cols-3">
                      ...
                      <label className="grid gap-1 text-sm font-bold text-slate-700">
                        메모
                        <input className="form-control bg-slate-50" value={holdingForm.memo} onChange={(event) => setHoldingForm({ ...holdingForm, memo: event.target.value })} />
                      </label>
                    </div>
                    <button className="mt-3 w-full rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white hover:bg-slate-800">
                      임시 포트폴리오에 담기
                    </button>
                  </form>
                )}
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-xs font-black uppercase tracking-wider text-slate-500">Draft Holdings</p>
                <h3 className="mt-1 text-2xl font-black text-slate-950">저장 전 종목 묶음</h3>
              </div>
              <button className="rounded-2xl bg-cyan-500 px-5 py-3 text-sm font-black text-slate-950 hover:bg-cyan-300" disabled={loading || !draftHoldings.length} onClick={savePortfolio} type="button">
                이 구성으로 포트폴리오 저장
              </button>
            </div>
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[760px] text-left text-sm">
                ...
                <tbody>
                  {draftHoldings.map((holding) => (
                    <tr className="border-b border-slate-100" key={holding.key}>
                      <td className="py-3 font-black text-slate-950">{holding.name}</td>
                      <td className="font-bold text-slate-500">{holding.ticker}</td>
                      ...
                      <td>
                        <button className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-700 hover:bg-rose-600 hover:text-white" onClick={() => removeDraftHolding(holding.key)} type="button">
                          제거
                        </button>
                      </td>
                    </tr>
              ...
              {!draftHoldings.length && <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">아직 담은 종목이 없습니다. 위에서 종목을 검색해 추가해주세요.</p>}
            </div>
          </section>
        ...
        <aside className="grid content-start gap-4">
          <section className="rounded-2xl border border-slate-800 bg-slate-950 p-4 text-white shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-xs font-black uppercase tracking-wider text-cyan-300">Saved</p>
                <h3 className="mt-1 text-xl font-black">저장된 포트폴리오</h3>
              </div>
              <span className="rounded-full bg-slate-800 px-3 py-1 text-xs font-black text-slate-300">{portfolios.length}개</span>
            </div>
            <div className="mt-4 grid max-h-[620px] gap-2 overflow-y-auto pr-1">
              {portfolios.map((portfolio) => {
                ...
                return (
                  <button
                    className={`rounded-2xl border p-4 text-left transition ${active ? "border-cyan-400 bg-cyan-400/10" : "border-slate-800 bg-slate-900 hover:border-slate-600"}`}
                    key={portfolio.id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <strong className="block truncate text-base font-black">{portfolio.name}</strong>
                        <span className="text-xs font-bold text-slate-500">{portfolio.stockCount}개 종목</span>
                      </div>
                    ...
                    <strong className="mt-3 block text-2xl font-black">{formatMoney(portfolio.totalValue)}</strong>
              ...
              {!portfolios.length && <p className="rounded-2xl bg-slate-900 p-4 text-sm font-bold text-slate-500">저장된 포트폴리오가 없습니다.</p>}
            </div>
          </section>
        </aside>
      </div>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h3 className="mt-1 text-2xl font-black text-slate-950">{detail?.name || "선택된 포트폴리오 없음"}</h3>
          {detail?.description && <p className="mt-1 text-sm font-bold text-slate-500">{detail.description}</p>}
        </div>
        {detail && (
          <button className="rounded-2xl bg-slate-950 px-4 py-2 text-sm font-black text-white hover:bg-rose-600" disabled={loading} onClick={onDelete} type="button">
            포트폴리오 삭제
          </button>
        )}
      </div>
      ...
      <div className="mt-5 overflow-x-auto">
        ...
        {!stocks.length && <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">선택한 포트폴리오에 등록된 종목이 없습니다.</p>}
      </div>
    </section>
  );
}
