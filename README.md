...
3. `OpenDART`  
   국내 기업 공시·재무 정보 보강용 공급자입니다. `OPENDART_API_KEY`가 있으면 연결재무제표를 우선 조회하고, 없을 때 별도재무제표를 조회합니다.

Fallback:

- 해외 종목·ETF 검색/현재가: FMP → Finnhub 또는 Yahoo Finance 검색·시세
- 해외 ETF 기본정보·구성종목·재무: FMP를 우선 사용하며, 키가 없거나 공급 범위 밖이면 Yahoo Finance 배당·시세까지만 보조하고 없는 값은 `정보 없음`으로 표시합니다.
- 국내 종목·ETF 검색/현재가: KRX Open API → 한국투자증권 Open API → 네이버 금융 시세
- 국내 ETF 상세: 네이버증권 공개 화면의 읽기 전용 ETF 기본정보·분배 이력·구성종목 응답으로 보강하고, Python/pykrx 경로는 구성종목 보조 수단으로만 사용합니다.
- 오래된 하드코딩 가격은 현재가로 대체하지 않습니다. 실시간 또는 최근 시세를 확인하지 못한 값은 `0원` 대신 `가격 확인 중` 또는 `정보 없음`으로 표시합니다.

공개 ETF API:

- `GET /api/market/etfs/search?market=ALL&keyword=SPY&limit=60`
- `GET /api/market/instruments/{ticker}?market=AUTO`
